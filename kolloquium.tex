...
\begin{frame}\scriptsize
  ...
  \begin{itemize}
    \item Finite-Volumen Methode für stationäre, incompressible Navier-Stokes Gleichungen
...
\begin{frame}\scriptsize
  ...
  \begin{itemize}
    \item Diskretisierung der Boussinesq Approximation des Auftriebsterms \(¿ \rho \beta (T - T_0 ) \)
    \item implizit oder explizit
  \end{itemize}
  \begin{itemize}
    \item Newton-Raphson Linearisierung konvektiver Term der Temperaturgleichung \\(Galpin et al.)
  \end{itemize}
\begin{align*}
...
\begin{align*}
  ...
  a_P^{T} T_{P} + \sum_{F \in NB(P)} a_F^{T} T_F + \underbrace{\sum_{j=1}^3 \left(a_P^{T,u_j} u_{P,j}+ \sum_{F \in NB(P)} a_F^{p,u_j} u_{F,j} \right) + a_P^{T,p} p_P + \sum_{F \in NB(P) } a_F^{T,p} p_F}_{\hbox{Newton-Raphson linearization}} &= b_{P,T}
\end{align*}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}
  ...
  \vspace{0.5cm}
  \begin{itemize}\scriptsize
    \item Behandlung der Blockränder nach Lilek et. al
    \item Nutzen von PETSc Datenstrukturen zur MPI-Parallelisierung
...
\begin{column}{5cm}
  ...
  \end{itemize}
  \vspace{3cm}
  \hspace*{-3cm} \scriptsize{$\color{fnbblue}\blacksquare$} an Stelle eines Eintrags steht eine dichtbesetze \(5x5\) Matrix
\end{column}
\end{columns}
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{columns}
 ...
 \begin{column}{6cm}
   \begin{itemize}\scriptsize
     ...
     \item starke Kopplung, da  Strömung getriben durch Temperaturdifferenzen 
     \item dreidimensionale Kavität mit beheizter Ost- und Westseite 
