...
\end{displaymath}
Here, \(N\) denotes the number of control volumes of the discretized solution domain \(V\). Each control volume has the volume \(V_n\). The numerical and the exact solution are evaluated at the center of each control volume for the error calculations. In order to calculate the normalized global error of the pressure \(p\), the the analytic solution \(\tilde{p}\) is modified as in section \ref{sec:sigularitytreatment}
\begin{displaymath}
